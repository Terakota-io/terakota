...
ships with the release as `cosign.pub`:
    ...
    cosign verify-blob --key cosign.pub --insecure-ignore-tlog=true \
      --signature terakota_v1.4.0_linux_amd64.tar.gz.sig \
      terakota_v1.4.0_linux_amd64.tar.gz
...
same two steps apply, with the package filename in place of the archive:
    ...
    sha256sum --check --ignore-missing SHA256SUMS
    cosign verify-blob --key cosign.pub --insecure-ignore-tlog=true \
      --signature terakota_v1.4.0_linux_amd64.deb.sig \
      terakota_v1.4.0_linux_amd64.deb
...
that by digest before publishing. You can check it yourself:
    ...
    dpkg-deb --fsys-tarfile terakota_v1.4.0_linux_amd64.deb | tar -xO ./usr/bin/terakota | sha256sum
    tar -xzOf terakota_v1.4.0_linux_amd64.tar.gz terakota | sha256sum
