...
`curl` sets them for `curl` instead, which does nothing:
    ...
    curl -fsSL https://terakota.io/install.sh | TERAKOTA_VERSION=v1.3.0 sh
...
**Linux packages (`.deb` / `.rpm`, since v1.1.0)**
    ...
    sudo dpkg -i terakota_v1.3.0_linux_amd64.deb
    sudo rpm -i  terakota_v1.3.0_linux_amd64.rpm
...
- Linux / macOS: `.tar.gz` (e.g. `terakota_v1.3.0_linux_amd64.tar.gz`,
  `terakota_v1.3.0_darwin_arm64.tar.gz`)
- Windows: `.zip` (e.g. `terakota_v1.3.0_windows_amd64.zip`)
...
Silicon, `darwin_amd64` on Intel Macs, `linux_amd64`/`linux_arm64` on Linux):
    ...
    tar -xzf terakota_v1.3.0_darwin_arm64.tar.gz
